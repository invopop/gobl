...
func TestNewHeader(t *testing.T) {
	...
	assert.NotPanics(t, func() {
		...
		h.Stamps = append(h.Stamps, &base.Stamp{})
	}, "header and meta hash should have been initialized")
}

func TestHeaderAddStamp(t *testing.T) {
	h := base.NewHeader()
	h.AddStamp(&base.Stamp{Provider: "foo", Value: "bar"})
	assert.Len(t, h.Stamps, 1)
	h.AddStamp(&base.Stamp{Provider: "foo", Value: "baz"})
	assert.Len(t, h.Stamps, 1)
	assert.Equal(t, "baz", h.Stamps[0].Value)
	h.AddStamp(&base.Stamp{Provider: "bar", Value: "bax"})
