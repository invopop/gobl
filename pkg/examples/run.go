...
// skip fragments are passed through to Sources. Each example runs as a named
// subtest. The addon(s) under test must already be registered in the test
// binary (e.g. via a blank import).
func Run(t *testing.T, root string, update bool, skip ...string) {
	t.Helper()

	files, err := Sources(root, skip...)
	if err != nil {
	...
	for _, path := range files {
		t.Run(path, func(t *testing.T) {
			...
			out, err := Convert(data, IsEnvelope(path))
			if err != nil {
