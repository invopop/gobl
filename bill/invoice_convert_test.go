...
func TestInvoiceConvertInto(t *testing.T) {
	...
	t.Run("conversion with item pricing", func(t *testing.T) {
		lines := []*bill.Line{
			{
		...
		i2, err := inv.ConvertInto(currency.USD)
		require.NoError(t, err)
		ip := i2.Lines[0].Item.Pricing
		assert.Equal(t, "100", ip.Per.String())
		assert.Equal(t, "134.4000", ip.Gross.String())
	...
	t.Run("conversion with item pricing and alt prices", func(t *testing.T) {
		lines := []*bill.Line{
			{
				Quantity: num.MakeAmount(1, 0),
				Item: &org.Item{
