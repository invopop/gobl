...
func TestLineCalculate(t *testing.T) {
	...
	t.Run("missing item", func(t *testing.T) {
		...
		assert.Nil(t, line.Total)
		assert.Nil(t, line.Sum)
	})
	...
	t.Run("lines with discount & charge low-precision precise", func(t *testing.T) {
		lines := []*Line{
			{
				Quantity: num.MakeAmount(20, 0),
				Item: &org.Item{
					Name:  "Test Item",
					Price: num.NewAmount(9033, 2),
				},
				Discounts: []*LineDiscount{
					{
		...
		assert.NoError(t, err)
		sum := calculateLineSum(lines, currency.EUR)
		assert.Equal(t, "1796.6000", sum.String())
		assert.Equal(t, "20.00", lines[0].Discounts[0].Amount.String())
		assert.Equal(t, "10.00", lines[0].Charges[0].Amount.String())
		assert.Equal(t, "1796.6000", lines[0].Total.String())
		roundLines(lines)
		assert.Equal(t, "1806.60", lines[0].Sum.String())
		assert.Equal(t, "20.00", lines[0].Discounts[0].Amount.String())
		assert.Equal(t, "10.00", lines[0].Charges[0].Amount.String())
		assert.Equal(t, "1796.60", lines[0].Total.String())
	})
