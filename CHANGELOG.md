...
Finally the `draft` flag has been removed. We've seen a lot of confusion with this field, not least because the inverse of "draft" isn't a word ("non-draft"), and it wasn't really adding much value that couldn't be handled in another way. The presence of envelope signatures now effectively implies that no further changes can be made to the contents.
