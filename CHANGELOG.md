...
- `net`: new bulk JWK Set endpoint at `/.well-known/jwks.json` alongside the existing per-kid `/.well-known/gobl/keys/<kid>` lookup, provided as a convenience for operators and third-party JOSE tooling. Keys are returned newest-first (sorted by `valid_from` descending, breaking ties by kid descending). `net.JWKSPath` and `Address.JWKSURL()` expose the well-known path. `cbc.URI` gains `Host()` / `Path()` accessors. CORS headers (`Access-Control-Allow-Origin: *`) are added to all server responses so browser-based JOSE tooling can fetch the JWKS.
- `dsig`: key IDs are now generated as UUIDv7 (time-ordered) instead of UUIDv4. Kid lexical order is now chronological — useful for the JWKS sort, and a small ergonomic win for operators inspecting on-disk keys.
