...
- `fr-ctc-flow2-v1` / `fr-ctc-flow6-v1` / `fr-ctc-flow10-v1`: **breaking**: validation is now **integrity-only**. Each addon keeps just the rules that protect its own data model — extension values must be registered codes (with the per-document-type partitions) and the document must be a supported shape (status/payment type, recognised status-line key). The French CTC **format/business rules** (`BR-FR-*`, `BR-FR-CDV-*`, `G1.*`/`G2.*`) — invoice-code format, SIREN-inbox requirements, VAT-percent whitelists, per-process-code reason allow-lists, final-invoice totals, exemption reasons, etc. — are **no longer enforced here**; they move to the `gobl.frctc` converter, which maps to the Schematron and owns detailed rejection reporting. Normalization, extensions and scenarios are unchanged.
