...
- `num`: `Amount.Multiply`, `Amount.Divide`, and `Amount.Rescale` now use `math/big` internally, fixing silent `int64` overflows that produced large negative results when handling values with many significant digits. Results that cannot be held at the requested exponent drop decimal places to fit, and division by zero returns zero.
