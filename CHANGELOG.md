...
- `head`: signing payload is now `{uuid, dig, iss, aud, iat}` — each signature carries the signer's GOBL Net identity (`iss`), an optional audience (`aud`), and an issued-at claim (`iat`). `Header.Sign`/`Envelope.Sign` now take `(key, iss, aud, opts ...head.SignOption)`; `Contains` is deprecated in favour of `Verify`.
