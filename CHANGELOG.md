...
- `ar-arca-v4`: Type T tourism invoice support (WSCT). New extension keys `ar-arca-tourism-relation` and `ar-arca-tourism-code`, doc types 195 (Invoice T), 196 (Debit Note T), 197 (Credit Note T). Rule 25 requires `ar-arca-tourism-relation` on type T invoices. Ordering/payment rules skipped for type T.
