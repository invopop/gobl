...
- `regimes/jp`: new tax regime for Japan (JP). Consumption Tax (消費税) modelled as
  a dedicated `JCT` category with a 10% standard and 8% reduced rate (plus
  historical rates), and qualified-invoice registration number validation
  (`T` + 13 digits) enforcing the NTA mod-9 check digit.
