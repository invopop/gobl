...
func TestIdentityRules(t *testing.T) {
	...
	t.Run("with invalid scope", func(t *testing.T) {
		...
		assert.Contains(t, faults.Error(), "identity scope when provided must be one of 'tax', 'legal', 'class', 'seller', or 'buyer'")
	})
...
func TestIdentityJSONSchema(t *testing.T) {
	...
	assert.Equal(t, org.IdentityScopeSeller, prop.OneOf[3].Const)
	assert.Equal(t, "Seller", prop.OneOf[3].Title)
	assert.Equal(t, org.IdentityScopeBuyer, prop.OneOf[4].Const)
	assert.Equal(t, "Buyer", prop.OneOf[4].Title)
}
