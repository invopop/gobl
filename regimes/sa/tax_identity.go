...
func validateTaxIdentity(tID *tax.Identity) error {
	return validation.ValidateStruct(tID,
		validation.Field(&tID.Code, validation.By(validateTINCode)),
	)
}

// validateTINCode checks that the TIN is a valid 15-digit format
// starting and ending with 3.
func validateTINCode(value any) error {
	...
	if !tinRegex.MatchString(val) {
		return errors.New("must be a 15-digit number starting and ending with 3")
	}
