...
var (
	...
	// ErrInvalidOrgNumberFormat indicates that the organization number is not in the correct format
	ErrInvalidOrgNumberFormat = errors.New("invalid organization number format")
	// ErrInvalidIndividualNumberFormat indicates that the person or coordination number is not in the correct format
	ErrInvalidIndividualNumberFormat = errors.New("invalid person or coordination number format")
	// ErrInvalidChecksum indicates that the identification number's checksum is invalid
	ErrInvalidChecksum = errors.New("invalid identification number checksum")
	...
	return validation.ValidateStruct(id,
		validation.Field(&id.Code,
			validation.By(func(value any) error {
				...
				// Normalize to digits only for Luhn check
				...
				case IdentityTypeOrgNr:
					if !orgNrRegex.MatchString(digitsOnly) {
						return ErrInvalidOrgNumberFormat
					}

				case IdentityTypePersonNr, IdentityTypeCoordinationNr:
					if !individualNrRegex.MatchString(code.String()) {
						return ErrInvalidIndividualNumberFormat
					}
				...
				if !internal.ValidateLuhn(digitsOnly) {
