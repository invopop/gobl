...
func billInvoiceRules() *rules.Set {
	return rules.For(new(bill.Invoice),
		...
		// Credit notes require preceding documents
		rules.When(bill.InvoiceTypeIn(bill.InvoiceTypeCreditNote),
			rules.Field("preceding",
				rules.Assert("02", "preceding documents are required for credit notes", is.Present),
			),
		),
		...
		rules.Field("preceding",
			rules.Each(
				rules.Field("issue_date",
					rules.Assert("03", "preceding issue date is required", is.Present),
				),
				rules.Field("code",
					rules.Assert("04", "preceding code is required", is.Present),
				),
			),
		),
		// Supplier validation
		rules.Field("supplier",
			...
			rules.Field("addresses",
				rules.Assert("07", "supplier addresses are required", is.Present),
				rules.Assert("08", "supplier first address must have a country",
					is.Func("first address country", firstAddressHasCountry),
				),
				rules.Assert("09", "supplier first address must have a street",
		...
		rules.Assert("14", "missing exemption note",
			is.Func("exemption note present", invoiceExemptionNotePresent),
		),
		rules.Assert("15", "too many exemption notes",
