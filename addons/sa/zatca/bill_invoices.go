	...
	return rules.For(new(bill.Invoice),
		...
		rules.When(
			...
			rules.Field("customer",
				rules.Assert("09", "customer must be present", is.Present),
				rules.Field("name",
					rules.Assert("10", "customer name must be present in the standard tax invoice (BR-KSA-42)", is.Present),
				),
				rules.Field("addresses",
					rules.Each(
						rules.Field("street",
							rules.Assert("11", "customer address must have a street name (BR-KSA-10)", is.Present),
						),
						rules.Field("locality",
							rules.Assert("12", "customer address must have a city name (BR-KSA-10)", is.Present),
						),
						rules.Field("country",
							rules.Assert("13", "customer address must have a country code (BR-KSA-10)", is.Present),
						),
					),
				),
				rules.Assert("14", "customer must have a valid identification scheme for standard invoices",
			...
			rules.Field("lines",
				rules.Each(
					rules.Field("taxes",
						rules.Assert("15", "line taxes are required for standard tax invoices and associated credit notes and debit notes (BR-KSA-52)", is.Present),
					),
					rules.Field("total",
						rules.Assert("16", "line total amount is required for standard tax invoices and associated credit notes and debit notes (BR-KSA-53)", is.Present),
					),
				),
			),
			rules.Field("delivery",
				rules.Assert("17", "delivery must be present", is.Present),
				rules.Field("date",
					rules.Assert("18", "delivery must have a supply date (BR-KSA-15)", is.Present),
				),
			),
		),
		...
		rules.When(
			...
			rules.Field("customer",
				rules.Field("tax_id",
					rules.Assert("19", "export invoices must not have buyer VAT registration number (BR-KSA-46)",
						is.Empty,
					),
				),
			),
		...
		rules.When(
			...
			rules.Field("delivery",
				rules.Assert("20", "delivery must be present for simplified and summary invoices", is.Present),
				rules.Field("period",
					rules.Assert("21", "supply must have a delivery period", is.Present),
					rules.Field("start",
						rules.Assert("22", "delivery start date must be present (BR-KSA-72)", is.Present),
					),
					rules.Field("end",
						rules.Assert("23", "delivery end date must be present (BR-KSA-72)", is.Present),
					),
				),
			),
			rules.Field("customer",
				rules.Assert("24", "customer must be present for simplified, summary invoices", is.Present),
				rules.Field("name",
					rules.Assert("25", "customer name must be present for simplified, summary invoices (BR-KSA-71)", is.Present),
				),
			),
		),
		...
		rules.When(
			...
			rules.Field("customer",
				rules.Field("identities",
					rules.Assert("26", "customer must have a national ID (NAT) when tax exemption is VATEX-SA-EDU or VATEX-SA-HEA (BR-KSA-49)",
		...
		rules.When(
			...
			rules.Field("customer",
				rules.Assert("27", "customer must be present for simplified or associated credit/debit note and EDU or HEA exemptions", is.Present),
				rules.Field("name",
					rules.Assert("28", "customer name must be present (BR-KSA-25)", is.Present),
				),
			),
		),
	)
