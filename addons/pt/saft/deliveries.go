...
func billDeliveryRules() *rules.Set {
	return rules.For(new(bill.Delivery),
		...
		rules.Field("despatch_date",
			rules.Assert("06", "despatch date is required", is.Present),
		),
		rules.Field("preceding",
			rules.Assert("07", "preceding must have at most one entry", is.Length(0, 1)),
			rules.Each(
				rules.Field("series",
					rules.Assert("08", "preceding series is required", is.Present),
				),
				rules.Field("code",
					rules.Assert("09", "preceding code is required", is.Present),
				),
			),
		),
	)
