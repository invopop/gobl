...
func TestOrgPartyNormalizePeppolEndpoint(t *testing.T) {
	...
	t.Run("normalizes a combined code+scheme value before copying", func(t *testing.T) {
		// org.Inbox.Normalize() splits "9920:x3157928m" into Scheme + Code
		// for peppol-keyed inboxes; the en16931 party normalizer then sees
		// already-split values. We exercise the combined path via the
		// addon's normalizer dispatch on the inbox first.
