...
err := rules.Validate(instr, tax.AddonContext(nfe.V4))
...
err := rules.Validate(instr, tax.AddonContext(nfe.V4))
assert.ErrorContains(t, err, "payment means extension is required")
})
...
err := rules.Validate(adv, tax.AddonContext(nfe.V4))
assert.NoError(t, err)
})
